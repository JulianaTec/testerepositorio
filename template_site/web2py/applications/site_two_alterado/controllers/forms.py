...
def car_model_form():
    ...
    form2=FORM('Car Model:', INPUT(_name='car_model'),
               'Base Price:', INPUT(_name='base_price'),
               INPUT(_type='submit'))
    ...
    if form2.accepts(request,session):
        # Retriving the form fields
        form2_car_model = form.vars.car_model
        form2_base_price = form.vars.base_price
        # Inserting in the database
        db.car_model.insert(car_model=form2_car_model, car_base_price=form2_base_price)
        # Tell the user about the insertion
        response.flash = 'New car: ' + form2_car_model
    elif form.errors:
        response.flash = 'Form has errors'
    else:
        response.flash = 'Please fill the form'
    ...
    return dict(car_model_form=form, car=form2)
